...
io.on('connection', function (socket)
{
    ...
    {
    	redisClient.keys('2*', function(err, keys)
    	{
	    	console.log(keys);
    	});
    	redisClient.del('2*', function(err, keys)
    	{
	    	console.log(keys);
    	});
    	redisClient.keys('2*', function(err, keys)
    	...
    	redisClient.get('2', function(err, res)
    	{
    		console.log("get res: "+res);
	    	if(res == 'null')
	    	{
		    	redisClient.set('2', socket.id, function(err)
		    	{
	...
	function generate()
	{
		for(var i=0; i<this.board.length; i++)
		{
			for(var j=0; j<this.board[i].length; j++)
			{
				...
				this.board[i][j].setId(i+j*this.board.length);
				redisClient.set(this.board[i][j].getId(), null);
